...
class QparserWorker(threading.Thread):
    ...
    def run(self):
        ...
        Logger.cPrint(Logger.INFO_TYPE, methodName,
                      f"Started worker '{self.wId}' on a inter-process socket '{backendBind}'")
        ...
        while True:
            ...
            try:
                ...
                # [SC] receive the question string and deserialize it
                qBlock = json.loads(worker.recv_string())
                Logger.cPrint(Logger.INFO_TYPE, methodName
                    , f"Worker '{self.wId}' received a request. Client: '{cId}'; Message: '{qBlock}'")
            ...
            try:
                ...
                Logger.cPrint(Logger.INFO_TYPE, methodName, f"Parsed the question '{qBlock['question']}'")

                cctAnnotator = TQConverter()
                cctAnnotator.cctToQuery(qParsed, True, True)
                cctAnnotator.cctToExpandedQuery(qParsed, False, False)
                Logger.cPrint(Logger.INFO_TYPE, methodName, f"Annotated the question '{qBlock['question']}'")
...
class QparserBroker(threading.Thread):
    ...
    def run(self):
        ...
        # [SC] creating a poller to forward client and worker messages
        poller = zmq.Poller()
        ...
        while True:
            ...
            try:
                ...
                if sockets.get(frontend) == zmq.POLLIN:
                    ...
                    # [SC] receive the question block json as string
                    qBlockStr = frontend.recv_string()
                    
                    Logger.cPrint(Logger.INFO_TYPE, methodName
                        , f"Forwarding a request from a client to a worker. Client: '{cId}'; Message: '{qBlockStr}'")
                    backend.send_string(cId, zmq.SNDMORE)
                    backend.send_string(qBlockStr)
...
def main():
    ...
    try:
        ...
        if workerInstanceCount <= 0:
            raise Exception(f"Invalid worker instance count '{workerInstanceCount}'. "
                            + f"Worker instance count should be 1 or higher integer number.")
    except Exception as e:
        Logger.cPrint(Logger.ERROR_TYPE, methodName,
                      f"============================ Invalid worker instance count")
        exc_info = sys.exc_info()
        Logger.cPrint(Logger.ERROR_TYPE, methodName, ''.join(traceback.format_exception(*exc_info)))
        
        Logger.cPrint(Logger.WARNING_TYPE, methodName,
                      f"Using the default worker instance count '{defWorkerInstanceCount}'")
