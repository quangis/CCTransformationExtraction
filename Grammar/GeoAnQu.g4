...
measure: location | (conAm coreC) |
         (aggre? DIGIT? (coreC 'and'?)+ (('of'|'for'|'to'|'during') DIGIT? 'each'? ('new'|extreDist)? (coreC|distBandNei))* weight?) |
...
|('conconpro' DIGIT ML)|('concovpro' DIGIT ML)|('covpro' DIGIT ML)|('proportion' DIGIT ML)|('concept' DIGIT);
